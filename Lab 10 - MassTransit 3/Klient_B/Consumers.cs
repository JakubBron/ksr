...
namespace Klient_A
{
    public class ConfirmationRequestConsumer : IConsumer<ConfirmationRequest>
    {
        public async Task Consume(ConsumeContext<ConfirmationRequest> context)
        {
            ...
            if (key == "Y")
            {
                ...
                Console.WriteLine($"Zam id={context.Message.OrderId} ok");
            }
            else
            {
                await context.Publish(new Rejection { OrderId = context.Message.OrderId });
                Console.WriteLine($"Zam id={context.Message.OrderId} odrzucono");
            }
        }
    }
    ...
    public class OrderAcceptedConsumer : IConsumer<OrderAccepted>
    {
        ...
        {
            Console.WriteLine($"Zam id={context.Message.OrderId} na {context.Message.Quantity} szt. przyjedzie!");
    ...
    public class OrderRejectedConsumer : IConsumer<OrderRejected>
    {
        ...
        {
            Console.WriteLine($"Zam id={context.Message.OrderId} na {context.Message.Quantity} szt. NIEprzyjedzie!");
            return Task.CompletedTask;
        }
    }
}